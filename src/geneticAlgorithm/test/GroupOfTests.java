...
public class GroupOfTests {
	...
	public GroupOfTests(String name, Class<?>[] testClasses, int runs){
		...
		for(int i = 0; i < testResults.length; i++){
			...
			if(remainder != 0) numberOfPointsToCalculate += 1;
			...
			//Average average
			double[] knownAverage = tr.averageAptitude;
			double[] unknownAverage = new double[numberOfPointsToCalculate];
			GenerationsToEvaluations.generationsToEvaluations(knownNodes, knownAverage, unknownNodes, unknownAverage);
			tr.averageAptitude = unknownAverage;
	...
	public void graphAverageAverage(){
		GroupOfTestsAverageAverageChart.graph(this.name, this.testResults, this.evaluationsPerGenerationToUse);
	}
	
	public void graphAverageBest(){
		GroupOfTestsAverageBestChart.graph(this.name, this.testResults, this.evaluationsPerGenerationToUse);
	}
